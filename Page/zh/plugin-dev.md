...
WinIsland 插件 API `0.3` 发布了原生 ABI v1。插件是直接加载进 WinIsland 进程的 Windows DLL，可以发布紧凑 Context、替换当前显示的媒体源、注册翻译，以及读取宿主当前状态。
...
1. 创建 `cdylib` crate，并依赖 `winisland-plugin-api = "0.3"`。
...
crate 版本 `0.3.x` 提供 ABI 版本 `1`。运行时兼容性由 `ABI_VERSION_1`、每个结构体的 `struct_size` 和服务表版本决定，而不是 DLL 加载时的 Rust crate 元数据。
