...
impl SettingsApp {
    ...
    fn build_notification_items(&self) -> Vec<SettingsItem> {
        let enabled = self.config.notification_enabled;
        let mut items = vec![
            SettingsItem::PageTitle { text: tr("tab_notification") },
            SettingsItem::SectionHeader { label: tr("section_notifications") },
            SettingsItem::GroupStart,
            SettingsItem::RowSwitch { label: tr("notification_enabled"), on: self.config.notification_enabled, enabled: true },
        ];
        
        if enabled {
            items.push(SettingsItem::RowSwitch { label: tr("notification_show_app"), on: self.config.notification_show_app_name, enabled: true });
            items.push(SettingsItem::RowSwitch { label: tr("notification_show_overview"), on: self.config.notification_show_overview, enabled: true });
            items.push(SettingsItem::RowStepper { label: tr("notification_duration"), value: format!("{:.0}", self.config.notification_duration), enabled: true });
        }
        
        items.push(SettingsItem::GroupEnd);
        
        if enabled {
            items.push(SettingsItem::SectionHeader { label: tr("notification_guide_title") });
            items.push(SettingsItem::GroupStart);
            items.push(SettingsItem::RowLabel { label: tr("notification_guide_step1") });
            items.push(SettingsItem::RowLabel { label: tr("notification_guide_step2") });
            items.push(SettingsItem::RowLabel { label: tr("notification_guide_step3") });
            items.push(SettingsItem::RowLabel { label: tr("notification_guide_step4") });
            items.push(SettingsItem::GroupEnd);
        }
    ...
    fn build_current_items(&self) -> Vec<SettingsItem> {
        match self.active_page {
            ...
            2 => self.build_notification_items(),
    ...
    fn sync_switch_targets(&mut self) {
        ...
        self.switch_anim.set_target(8, self.config.notification_enabled);
        self.switch_anim.set_target(9, self.config.notification_show_app_name);
    }
    ...
    fn draw_sidebar(&self, canvas: &skia_safe::Canvas) {
        ...
        let pages = [tr("tab_general"), tr("tab_music"), tr("tab_notification"), tr("tab_about")];
        let start_y = 20.0;
    ...
    fn get_page_anim(&self) -> SwitchAnimator {
        match self.active_page {
            ...
            2 => {
                SwitchAnimator::new_with_anims(&self.switch_anim, &[8, 9])
            }
    ...
    fn handle_click(&mut self) {
        ...
        match self.active_page {
            ...
            2 => self.handle_notification_click(&items, content_x, content_y, content_w),
    ...
    fn handle_notification_click(&mut self, items: &[SettingsItem], mx: f32, my: f32, width: f32) {
        let result = hit_test(items, mx, my, CONTENT_START_Y, width);
        let mut changed = false;
        ...
        match result {
            ClickResult::Switch(idx) => {
                match idx {
                    ...
                    1 => if self.config.notification_enabled { self.config.notification_show_app_name = !self.config.notification_show_app_name },
                    2 => if self.config.notification_enabled { self.config.notification_show_overview = !self.config.notification_show_overview },
                    _ => {}
                }
                self.sync_switch_targets();
                changed = true;
            ...
            ClickResult::StepperDec(idx) | ClickResult::StepperInc(idx) => {
                ...
                if let Some(item) = items.get(idx) {
                    if let SettingsItem::RowStepper { label, .. } = item {
                        if label == &tr("notification_duration") && self.config.notification_enabled {
                            if is_dec { self.config.notification_duration = (self.config.notification_duration - 1.0).max(1.0); }
                            else { self.config.notification_duration = (self.config.notification_duration + 1.0).min(30.0); }
                            changed = true;
                        }
                    }
                }
