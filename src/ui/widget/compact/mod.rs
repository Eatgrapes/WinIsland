...
fn slot_widget(layout: &[CompactWidgetSlot], slot: usize) -> Option<CompactWidgetKind> {
    layout
        .iter()
...
pub(crate) fn has_center_widget(layout: &[CompactWidgetSlot]) -> bool {
    slot_widget(layout, CENTER_SLOT).is_some()
}
...
fn minimum_layout_width(layout: &[CompactWidgetSlot]) -> f32 {
    let left_width = slot_widget(layout, LEFT_SLOT).map(widget_width);
    let center_width = slot_widget(layout, CENTER_SLOT).map(widget_width);
    let right_width = slot_widget(layout, RIGHT_SLOT).map(widget_width);
...
pub(crate) fn target_width(
    ...
    let center_occupied = has_center_widget(layout);
    ...
    let (left_extension, right_extension) = side_extensions(layout, has_center_content);
    (center_width + left_extension + right_extension)
        .max(base_width)
        .max(minimum_layout_width(layout))
}
...
pub(crate) fn draw(
    ...
    for slot in 0..COMPACT_WIDGET_SLOTS {
        let Some(widget) = slot_widget(layout, slot) else {
            continue;
        };
    ...
    if has_center_content {
        draw_separators(canvas, layout, rect, scale, alpha);
    }
}
...
fn draw_separators(
    ...
    let (left_extension, right_extension) = side_extensions(layout, true);
