    ...
    while i < items.len() {
        ...
        match item {
            ...
            SettingsItem::FontPreview { has_custom_font } => {
                let preview_h = 90.0;
                let preview_y = y + (item.height() - preview_h) / 2.0;
                let col_w = (content_w - GROUP_INNER_PAD) / 2.0;
                let fm = FontManager::global();

                let mut label_paint = Paint::default();
                label_paint.set_color(COLOR_TEXT_SEC);
                let mut text_paint = Paint::default();
                text_paint.set_color(COLOR_TEXT_PRI);

                let dft_label = crate::core::i18n::tr("font_preview_default");
                fm.draw_text_with_default_font(
                    canvas,
                    &dft_label,
                    (CONTENT_PADDING, preview_y),
                    12.0,
                    false,
                    &label_paint,
                );
                fm.draw_text_with_default_font(
                    canvas,
                    "中文 English 日本語",
                    (CONTENT_PADDING, preview_y + 20.0),
                    13.0,
                    false,
                    &text_paint,
                );
                if *has_custom_font {
                    let cst_label = crate::core::i18n::tr("font_preview_custom");
                    fm.draw_text_with_custom_font(
                        canvas,
                        &cst_label,
                        (CONTENT_PADDING + col_w, preview_y),
                        12.0,
                        false,
                        &label_paint,
                    );
                    fm.draw_text_with_custom_font(
                        canvas,
                        "中文 English 日本語",
                        (CONTENT_PADDING + col_w, preview_y + 20.0),
                        13.0,
                        false,
                        &text_paint,
                    );
