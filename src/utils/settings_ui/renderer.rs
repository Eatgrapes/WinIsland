...
fn count_group_rows(items: &[SettingsItem], start: usize) -> usize {
    ...
    let mut i = start;
    while i < items.len() {
        if matches!(items[i], SettingsItem::GroupEnd) { break; }
        if items[i].is_row() { count += 1; }
        i += 1;
    }
    count
}

fn draw_row_hover(canvas: &Canvas, y: f32, content_w: f32, row_idx: usize, in_group: bool, hover_anims: &AnimPool) {
...
pub fn draw_items(canvas: &Canvas, items: &[SettingsItem], start_y: f32, width: f32, anims: &SwitchAnimator, hover_anims: &AnimPool) {
    let fm = FontManager::global();
    let mut y = start_y;
    ...
    while i < items.len() {
        ...
        match item {
            ...
            SettingsItem::RowFontPicker { label, btn_label, reset_label } => {
                ...
                draw_pill_btn(canvas, sel_x, cy - 13.0, sel_w, 26.0, btn_label, COLOR_TEXT_PRI, COLOR_CARD_HIGHLIGHT);

                if let Some(rl) = reset_label {
                    let rst_w: f32 = 60.0;
                    let rst_x = sel_x - rst_w - 6.0;
                    draw_pill_btn(canvas, rst_x, cy - 13.0, rst_w, 26.0, rl, COLOR_DANGER, COLOR_CARD_HIGHLIGHT);
                }
            ...
            SettingsItem::RowSourceSelect { label, options, enabled } => {
                ...
                p.set_color(if *enabled { COLOR_TEXT_PRI } else { COLOR_TEXT_SEC });
                ...
                p.set_color(if *enabled { COLOR_TEXT_SEC } else { COLOR_DISABLED });
